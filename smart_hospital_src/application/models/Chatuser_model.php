...
class Chatuser_model extends MY_Model
{
    ...
    public function searchForUser($keyword, $chat_user_id, $login_id, $user_type = 'staff')
    {
        ...
        if ($user_type == 'staff') {
            ...
            // Patients may only contact staff — never other patients (privacy)
            $sql = "SELECT staff.employee_id, staff.id as `staff_id`, Null as `patient_id`, staff.name, staff.surname, Null as `first_name`, staff.image FROM `staff` WHERE staff.name LIKE '%" . $keyword . "%' and is_active= 1 and id NOT IN(SELECT chat_users.staff_id FROM `chat_users` inner JOIN (SELECT chat_connections.id, CASE WHEN chat_user_one =" . $chat_user_id . " THEN chat_user_two ELSE chat_connections.chat_user_one END as 'chat_user_id' FROM `chat_connections` WHERE (chat_user_one=" . $chat_user_id . " or chat_user_two=" . $chat_user_id . ")) as chat_connections on chat_connections.chat_user_id=chat_users.id WHERE staff_id IS NOT NULL)";
        }
    ...
    {
        $chat_connection_id = (int) $chat_connection_id;
    ...
    public function getChatUserDetail_old($chat_user_id)
    {
        $chat_user_id = (int) $chat_user_id;
        $sql = "SELECT chat_users.id as `chat_user_id`, chat_users.user_type, chat_users.patient_id, chat_users.staff_id, 
                (CASE 
                    WHEN chat_users.user_type = 'patient' THEN patients.patient_name 
                    ELSE (CASE WHEN staff.surname = '' THEN staff.name ELSE CONCAT(staff.name, ' ', staff.surname) END) 
                END) as name,
                (CASE 
                    WHEN chat_users.user_type = 'patient' THEN patients.image 
                    ELSE staff.image 
                END) as image 
                FROM `chat_users` 
                LEFT JOIN patients ON patients.id = chat_users.patient_id 
                LEFT JOIN staff ON staff.id = chat_users.staff_id 
                WHERE chat_users.id = " . $chat_user_id;
        $query     = $this->db->query($sql);
        $chat_user = $query->row();
    ...
    public function addNewUserForpatient($first_entry, $insert_data, $id, $insert_message, $panel = "patient")
    {
        ...
        $this->db->where('user_type', $first_entry['user_type']);
        $q = $this->db->get('chat_users');
        if (isset($insert_data['user_type']) && $insert_data['user_type'] == 'patient') {
            $this->db->where('patient_id', isset($insert_data['patient_id']) ? $insert_data['patient_id'] : null);
        } else {
            $this->db->where('staff_id', isset($insert_data['staff_id']) ? $insert_data['staff_id'] : null);
        }
        $this->db->where('user_type', $insert_data['user_type']);
        $q1 = $this->db->get('chat_users');
        if ($q->num_rows() > 0 && $q1->num_rows() > 0) {
    ...
    public function mynewuser($user_id, $userlist)
    {
        ...
        $ids     = "";
        if (!empty($userlist)) {
            $escaped_ids = array_map('intval', $userlist);
            $ids         = " and id NOT IN (" . implode(',', $escaped_ids) . ") ";
        }

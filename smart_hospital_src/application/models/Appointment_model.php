...
class Appointment_model extends MY_Model
{
    ...
    public function getAllappointmentRecord($type)
    {
        ...
        $this->datatables
            ->select('appointment.amount as appointment_amount,appointment_payment.standard_amount,appointment.appointment_status,appointment.id,appointment.date,appointment.source,IFNULL((SELECT SUM(amount) FROM transactions WHERE transactions.appointment_id = appointment.id AND transactions.type = "payment"), IFNULL(appointment_payment.paid_amount, 0)) as paid_amount,IFNULL((SELECT SUM(amount) FROM transactions WHERE transactions.appointment_id = appointment.id AND transactions.type = "refund"), 0) as refund_amount,appointment_payment.discount_percentage,staff.id as sid,staff.name,patients.id as pid, patients.patient_name as patient_name,patients.gender as gender, patients.email as email, patients.mobileno as mobileno,staff.surname,staff.employee_id,appoint_priority.appoint_priority as priorityname,appointment.live_consult,created_by_staff.name as created_by_name, created_by_staff.surname as created_by_surname,created_by_staff.employee_id as created_by_employee_id' . $field_variable)  

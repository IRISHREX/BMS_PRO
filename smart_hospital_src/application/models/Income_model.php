...
class Income_model extends MY_Model
{
    ...
    public function search($text = null, $start_date = null, $end_date = null)
    {
        if (!empty($text)) {
            ...
            $this->db->join(
                "income_head",
                "income.inc_head_id = income_head.id",
            );
    ...
    {
        $query =
            "SELECT sum(amount) as `amount` FROM `income` where date=" .
            $this->db->escape($date);
    ...
    {
        $query =
            "SELECT sum(amount) as `amount` FROM `income` where date between " .
            $this->db->escape($date_from) .
            " and " .
            $this->db->escape($date_to);
        $query = $this->db->query($query);
        return $query->row();
    }
    ...
    public function searchincomegroup($search_type, $head_id = null)
    {
        ...
        if ($search_type == "period") {
            $this->form_validation->set_rules(
                "date_from",
                $this->lang->line("date_from"),
                "trim|required|xss_clean",
            );
            $this->form_validation->set_rules(
                "date_to",
                $this->lang->line("date_to"),
                "trim|required|xss_clean",
            );
            ...
            $last_date = date(
                "Y-" . $month . "-" . date("t", strtotime($first_date)),
            );
            $start_date = $first_date;
            $end_date = $last_date;
        ...
        $this->db
            ->select(
                'GROUP_CONCAT(income.id,"@",income.name,"@",income.invoice_no,"@",income.date,"@",income.amount) as income, income_head.income_category,sum(income.amount) as total_amount,' .
                    $field_variable,
            )
            ->from("income");
        $this->db->join("income_head", "income.inc_head_id = income_head.id");
    ...
    public function getAllRecord()
    {
        ...
        $this->datatables
            ...
            ->searchable(
                "income.name,income.invoice_no,income.date,income.note,income_head.income_category,null" .
                    $custom_field_column,
            )
            ->orderable(
            ...
            ->join(
                ...
                "generated_by_staff.id = income.generated_by",
                "left",
            )
            ->sort("income.id", "desc")
            ->from("income");

...
class Radio_model extends MY_Model
{
    ...
    {
        $this->datatables
            ->select('radiology_billing.*,( SELECT IFNULL(SUM(CASE WHEN transactions.type="payment" THEN transactions.amount ELSE 0 END),0) from transactions WHERE transactions.radiology_billing_id=radiology_billing.id ) as gross_paid,IFNULL((SELECT SUM(amount) FROM transactions WHERE radiology_billing_id=radiology_billing.id AND type="refund"),0) as refund_amount,SUM(CASE WHEN transactions.type="payment" THEN transactions.amount WHEN transactions.type="refund" THEN -transactions.amount ELSE 0 END) as paid_amount,patients.patient_name,patients.id as patient_unique_id,staff.name,staff.surname,staff.employee_id')
            ->join('patients', 'patients.id = radiology_billing.patient_id', 'left')
    ...
    public function getRadiologyChargeByPatient($id,$patient_id=null)
    {
        ...
        WHERE `radio`.`id` = ".$this->db->escape($id).") d ORDER BY d.temp_col DESC LIMIT 1";
        $query  = $this->db->query($sql);

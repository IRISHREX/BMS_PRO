...
class Patient_model extends MY_Model
{
    ...
    public function getallinvestigation($case_reference_id){
        ...
        $query = $this->db->query("select pathology_report.id as report_id,pathology_report.pathology_bill_id,CONVERT(pathology.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(pathology.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name,pathology.report_days,pathology.id as pid,pathology.charge_id as cid,staff.name,staff.surname,collection_specialist_staff.name as `collection_specialist_staff_name`,collection_specialist_staff.surname as `collection_specialist_staff_surname`,collection_specialist_staff.employee_id as `collection_specialist_staff_employee_id`,approved_by_staff.name as `approved_by_staff_name`,approved_by_staff.surname as `approved_by_staff_surname`,approved_by_staff.employee_id as `approved_by_staff_employee_id`, CONVERT('pathology' USING utf8mb4) COLLATE utf8mb4_unicode_ci as type, CONVERT(pathology_report.pathology_center USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_center, pathology_report.collection_date,pathology_report.reporting_date,CONVERT(pathology_report.parameter_update USING utf8mb4) COLLATE utf8mb4_unicode_ci as parameter_update from pathology_billing inner join pathology_report on pathology_report.pathology_bill_id = pathology_billing.id inner join pathology on pathology_report.pathology_id = pathology.id left join staff on staff.id = pathology_billing.doctor_id left join staff as collection_specialist_staff on collection_specialist_staff.id = pathology_report.collection_specialist left join staff as approved_by_staff on approved_by_staff.id = pathology_report.approved_by where pathology_billing.case_reference_id= ".$this->db->escape($case_reference_id) . "
            union all 
            select radiology_report.id as report_id, radiology_report.radiology_bill_id,CONVERT(radio.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(radio.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name,radio.report_days,radio.id as pid,radio.charge_id as cid,staff.name,staff.surname,collection_specialist_staff.name as `collection_specialist_staff_name`,collection_specialist_staff.surname as `collection_specialist_staff_surname`,collection_specialist_staff.employee_id as `collection_specialist_staff_employee_id`,approved_by_staff.name as `approved_by_staff_name`,approved_by_staff.surname as `approved_by_staff_surname`,approved_by_staff.employee_id as `approved_by_staff_employee_id`, CONVERT('radiology' USING utf8mb4) COLLATE utf8mb4_unicode_ci as type,CONVERT(radiology_report.radiology_center USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_center,radiology_report.collection_date,radiology_report.reporting_date,CONVERT(radiology_report.parameter_update USING utf8mb4) COLLATE utf8mb4_unicode_ci as parameter_update  from radiology_billing inner join radiology_report on radiology_report.radiology_bill_id = radiology_billing.id inner join radio on radiology_report.radiology_id = radio.id left join staff on staff.id = radiology_report.consultant_doctor left join staff as collection_specialist_staff on collection_specialist_staff.id = radiology_report.collection_specialist left join staff as approved_by_staff on approved_by_staff.id = radiology_report.approved_by where radiology_billing.case_reference_id=".$this->db->escape($case_reference_id)." "  );
           $result = $query->result_array();
          return $result ;
    }
    ...
    public function allinvestigationbypatientid($patient_id){
        ...
        $query = $this->db->query("select pathology_report.id as report_id,pathology_report.pathology_bill_id,CONVERT(pathology.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(pathology.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name,pathology.report_days,pathology.id as pid,pathology.charge_id as cid,staff.name,staff.surname,collection_specialist_staff.name as `collection_specialist_staff_name`,collection_specialist_staff.surname as `collection_specialist_staff_surname`,collection_specialist_staff.employee_id as `collection_specialist_staff_employee_id`,approved_by_staff.name as `approved_by_staff_name`,approved_by_staff.surname as `approved_by_staff_surname`,approved_by_staff.employee_id as `approved_by_staff_employee_id`, CONVERT('pathology' USING utf8mb4) COLLATE utf8mb4_unicode_ci as type, CONVERT(pathology_report.pathology_center USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_center, pathology_report.collection_date,pathology_report.reporting_date,CONVERT(pathology_report.parameter_update USING utf8mb4) COLLATE utf8mb4_unicode_ci as parameter_update,CONVERT(pathology_billing.case_reference_id USING utf8mb4) COLLATE utf8mb4_unicode_ci as case_reference_id from pathology_billing inner join pathology_report on pathology_report.pathology_bill_id = pathology_billing.id inner join pathology on pathology_report.pathology_id = pathology.id left join staff on staff.id = pathology_billing.doctor_id left join staff as collection_specialist_staff on collection_specialist_staff.id = pathology_report.collection_specialist left join staff as approved_by_staff on approved_by_staff.id = pathology_report.approved_by where pathology_billing.patient_id= ".$this->db->escape($patient_id) . "
            union all 
            select radiology_report.id as report_id, radiology_report.radiology_bill_id,CONVERT(radio.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(radio.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name,radio.report_days,radio.id as pid,radio.charge_id as cid,staff.name,staff.surname,collection_specialist_staff.name as `collection_specialist_staff_name`,collection_specialist_staff.surname as `collection_specialist_staff_surname`,collection_specialist_staff.employee_id as `collection_specialist_staff_employee_id`,approved_by_staff.name as `approved_by_staff_name`,approved_by_staff.surname as `approved_by_staff_surname`,approved_by_staff.employee_id as `approved_by_staff_employee_id`, CONVERT('radiology' USING utf8mb4) COLLATE utf8mb4_unicode_ci as type,CONVERT(radiology_report.radiology_center USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_center,radiology_report.collection_date,radiology_report.reporting_date,CONVERT(radiology_report.parameter_update USING utf8mb4) COLLATE utf8mb4_unicode_ci as parameter_update,CONVERT(radiology_billing.case_reference_id USING utf8mb4) COLLATE utf8mb4_unicode_ci as case_reference_id  from radiology_billing inner join radiology_report on radiology_report.radiology_bill_id = radiology_billing.id inner join radio on radiology_report.radiology_id = radio.id left join staff on staff.id = radiology_report.consultant_doctor left join staff as collection_specialist_staff on collection_specialist_staff.id = radiology_report.collection_specialist left join staff as approved_by_staff on approved_by_staff.id = radiology_report.approved_by where radiology_billing.patient_id=".$this->db->escape($patient_id));
           $result = $query->result_array();
          return $result ;
    }
    ...
    {
      $query=$this->db->query("select CONVERT(pathology.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(pathology.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name
            ...
            inner join pathology  on pathology_report.pathology_id = pathology.id
             where pathology_billing.patient_id=".$this->db->escape($patient_id)."
             union all 
             select CONVERT(radio.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(radio.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name
            ...
            inner join radio  on radiology_report.radiology_id =radio.id
             where radiology_billing.patient_id=".$this->db->escape($patient_id)."  ");
    ...
    public function getlabinvestigationbycaseid($case_reference_id)
    {
      $query=$this->db->query("select CONVERT(pathology.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(pathology.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name
            ...
            inner join pathology_report  on pathology_billing.id= pathology_report.pathology_bill_id
            inner join pathology  on pathology_report.pathology_id = pathology.id
             where opd_details.case_reference_id =".$this->db->escape($case_reference_id)."
             union all 
             select CONVERT(radio.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(radio.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name
            from
            opd_details 
    ...
    public function getpatientoverview($patient_id){
        ...
        $patient_id_escaped = $this->db->escape($patient_id);
        $query=$this->db->query("select CONVERT(pathology.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(pathology.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name from pathology_billing inner join pathology_report on pathology_billing.id = pathology_report.pathology_bill_id inner join pathology on pathology_report.pathology_id = pathology.id
             where pathology_billing.patient_id=".$patient_id_escaped." union all select CONVERT(radio.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name,CONVERT(radio.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name from radiology_billing inner join radiology_report on radiology_billing.id = radiology_report.radiology_bill_id inner join radio on radiology_report.radiology_id = radio.id where radiology_billing.patient_id=".$patient_id_escaped." limit 5");
    ...
    public function getpatientoverviewbycaseid($case_reference_id){
        ...
        $case_id = $this->db->escape($case_reference_id);
        $query = $this->db->query("select CONVERT(pathology.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name, CONVERT(pathology.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name from opd_details left join pathology_billing on opd_details.patient_id = pathology_billing.patient_id inner join pathology_report on pathology_billing.id = pathology_report.pathology_bill_id inner join pathology on pathology_report.pathology_id = pathology.id
             where opd_details.case_reference_id = " . $case_id . "
             union all select CONVERT(radio.test_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as test_name, CONVERT(radio.short_name USING utf8mb4) COLLATE utf8mb4_unicode_ci as short_name from opd_details left join radiology_billing on opd_details.patient_id = radiology_billing.patient_id inner join radiology_report on radiology_billing.id = radiology_report.radiology_bill_id inner join radio on radiology_report.radiology_id = radio.id where opd_details.case_reference_id = " . $case_id);
    ...
    public function getallopdvisits($type, $show_antenatal = false, $caller = 'bill_opd')
    {
        ...
        if ($caller === 'patient_search') {
            $orderable_cols = array(
                ...
                'referral_person.name',           // [7] Referral Person Name
                'visit_details.symptoms',         // [8] Symptoms
            );
            if ($show_antenatal) {
        ...
        $this->datatables
            ->select('opd_details.case_reference_id,opd_details.id as opd_id,opd_details.patient_id as patientid,opd_details.is_ipd_moved,max(visit_details.id) as visit_id,visit_details.appointment_date,visit_details.refference,visit_details.symptoms,patients.id as pid,patients.patient_name,staff.id as staff_id,staff.name,staff.surname,staff.employee_id,consult_charges.standard_charge,patient_charges.apply_charge,visit_details.is_antenatal,generated_by_staff.name as generated_byname,generated_by_staff.surname as generated_bysurname,generated_by_staff.employee_id as generated_byemployee_id,patients.gender,patients.guardian_name,patients.mobileno,max(visit_details.appointment_date) as last_visit,count(opd_details.patient_id) as total_visit,(SELECT referral_person.name FROM referral_payment JOIN referral_person ON referral_person.id = referral_payment.referral_person_id WHERE referral_payment.billing_id = visit_details.id AND referral_payment.referral_type = 1 LIMIT 1) as referral_person_name,' . $field_variable)
            ->join('visit_details', 'opd_details.id = visit_details.opd_details_id')
            ->join('staff', 'staff.id = visit_details.cons_doctor', "inner")
            ->join('staff as generated_by_staff', 'generated_by_staff.id = opd_details.generated_by', "left")
            ->join('patients', 'patients.id = opd_details.patient_id', "inner")

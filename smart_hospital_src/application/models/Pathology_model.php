...
class Pathology_model extends MY_Model
{
    ...
    {
        $this->datatables
            ->select('pathology_billing.*,SUM(CASE WHEN transactions.type="payment" THEN transactions.amount WHEN transactions.type="refund" THEN -transactions.amount ELSE 0 END) as paid_amount,patients.patient_name,IFNULL((SELECT SUM(amount) FROM transactions WHERE pathology_billing_id=pathology_billing.id AND type="refund"),0) as refund_amount,patients.id as patient_unique_id,staff.name,staff.surname,staff.employee_id')
            ->join('patients', 'patients.id = pathology_billing.patient_id', 'left')
            ->join('staff', 'staff.id = pathology_billing.doctor_id', 'left')

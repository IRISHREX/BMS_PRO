...
class Pathology_model extends MY_Model
{
    ...
    public function getAllpathologybillRecord()
    {
        ...
        $this->datatables
            ->select('pathology_billing.*,( SELECT IFNULL(SUM(CASE WHEN transactions.type="payment" THEN transactions.amount ELSE 0 END),0) from transactions WHERE transactions.pathology_billing_id=pathology_billing.id ) as gross_paid,( SELECT IFNULL(SUM(CASE WHEN transactions.type="refund" THEN transactions.amount ELSE 0 END),0) from transactions WHERE transactions.pathology_billing_id=pathology_billing.id ) as refund_amount,( SELECT IFNULL(SUM(CASE WHEN transactions.type="payment" THEN transactions.amount WHEN transactions.type="refund" THEN -transactions.amount ELSE 0 END),0) from transactions WHERE transactions.pathology_billing_id=pathology_billing.id ) as paid_amount,patients.patient_name,patients.id as pid,staff.name,staff.surname,staff.employee_id,generated_by_staff.name as generated_byname,generated_by_staff.surname as generated_bysurname,generated_by_staff.employee_id as generated_byemployee_id, (SELECT referral_person.name FROM referral_payment JOIN referral_person ON referral_person.id = referral_payment.referral_person_id WHERE referral_payment.billing_id = pathology_billing.id AND referral_payment.referral_type = 4 LIMIT 1) as referral_person_name'.$field_variable)
    ...
    {
        $this->datatables
            ->select('pathology_billing.*,( SELECT IFNULL(SUM(CASE WHEN transactions.type="payment" THEN transactions.amount ELSE 0 END),0) from transactions WHERE transactions.pathology_billing_id=pathology_billing.id ) as gross_paid,IFNULL((SELECT SUM(amount) FROM transactions WHERE pathology_billing_id=pathology_billing.id AND type="refund"),0) as refund_amount,SUM(CASE WHEN transactions.type="payment" THEN transactions.amount WHEN transactions.type="refund" THEN -transactions.amount ELSE 0 END) as paid_amount,patients.patient_name,patients.id as patient_unique_id,staff.name,staff.surname,staff.employee_id')
            ->join('patients', 'patients.id = pathology_billing.patient_id', 'left')
            ->join('staff', 'staff.id = pathology_billing.doctor_id', 'left')
    ...
    {   
        $query=$this->db->select('pathology_billing.*,IFNULL((SELECT SUM(CASE WHEN transactions.type="payment" THEN transactions.amount WHEN transactions.type="refund" THEN -transactions.amount ELSE 0 END) from transactions WHERE transactions.pathology_billing_id=pathology_billing.id),0) as `amount_paid`,patients.patient_name,patients.id as patient_id')
            ->join('patients', 'patients.id = pathology_billing.patient_id', 'left')
            ->where('pathology_billing.case_reference_id', $case_id)           
          ->get('pathology_billing');
        return $query->result();
    ...
    {      
        ...
        WHERE  `pathology`.`id` = ".$this->db->escape($id).") d ORDER BY d.temp_col DESC LIMIT 1";
        $query  = $this->db->query($sql);        
        if ($query->num_rows() > 0) {
            $result = $query->row();          
            return $result;
    ...
    { 
		...
		$query = $this->db->select('pathology_billing.*, (SELECT referral_person.name FROM referral_payment JOIN referral_person ON referral_person.id = referral_payment.referral_person_id WHERE referral_payment.billing_id = pathology_billing.id AND referral_payment.referral_type = 4 LIMIT 1) as referral_person_name, (SELECT referral_person_id FROM referral_payment WHERE billing_id=pathology_billing.id and referral_type=4 limit 1) as referral_person_id, blood_bank_products.name as blood_group_name,IFNULL((SELECT SUM(amount) FROM transactions WHERE pathology_billing_id=pathology_billing.id AND type="payment"),0) as total_deposit,patients.patient_name,IFNULL((SELECT SUM(amount) FROM transactions WHERE pathology_billing_id=pathology_billing.id AND type="refund"),0) as refund_amount,patients.id as patient_unique_id,patients.dob,patients.as_of_date,patients.age,patients.month,patients.day,patients.gender,patients.blood_group,patients.mobileno,patients.email,patients.address,staff.name,staff.surname,staff.employee_id,transactions.payment_mode,transactions.amount,transactions.cheque_no,transactions.cheque_date,transactions.note as `transaction_note`,staff_roles.role_id as staff_roles_id,org.organisation_name,pathology_billing.insurance_validity,pathology_billing.insurance_id'.$field_variable)
    ...
    public function getTestReportBatchPatho($patient_id)
    {
        ...
        $field_variable = (empty($field_var_array))? "": ",".implode(',', $field_var_array);
        $custom_field_column = (empty($custom_field_column_array))? "": ",".implode(',', $custom_field_column_array);
        $this->db->select('pathology_billing.*,SUM(CASE WHEN transactions.type="payment" THEN transactions.amount WHEN transactions.type="refund" THEN -transactions.amount ELSE 0 END) as paid_amount,patients.patient_name,patients.id as pid,staff.name,staff.surname,staff.employee_id'.$field_variable);

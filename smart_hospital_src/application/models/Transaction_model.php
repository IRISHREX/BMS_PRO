...
{    
   ...
   public function ipdopdPaymentByCaseId($case_id)
    ...
    public function radiologybillreportsRecord($data) {
            ...
            $sql="select  radiology_billing.discount_percentage,radiology_report.tax_percentage,radiology_report.apply_charge,lab.lab_name,'radiology' head,'radiology_billing' module_no,radiology_billing.patient_id,patients.patient_name,radiology_billing.note,radiology_billing.date,radiology_billing.doctor_name,radiology_report.radiology_bill_id as module_id,radiology_billing.net_amount,radiology_billing.total,charges.standard_charge,staff.name,staff.surname,staff.employee_id,radio.test_name,radio.short_name,radiology_report.is_canceled,radiology_report.approved_by,radiology_report.collection_specialist,radiology_billing.status as bill_status,( SELECT IFNULL(SUM(CASE WHEN transactions.type=\"payment\" THEN transactions.amount WHEN transactions.type=\"refund\" THEN -transactions.amount ELSE 0 END),0) from transactions WHERE transactions.radiology_billing_id=radiology_billing.id ) as paid_amount".$field_variable." from radiology_billing 
	...
	public function dtradiologybalancereport($data) {
			...
			( SELECT IFNULL(SUM(CASE WHEN transactions.type=\"payment\" THEN transactions.amount WHEN transactions.type=\"refund\" THEN -transactions.amount ELSE 0 END),0) from transactions WHERE transactions.radiology_billing_id=radiology_billing.id ) as paid_amount, radiology_billing.net_amount, radiology_billing.discount, radiology_billing.discount_percentage			

...
class dashboard extends Patient_Controller
{
    ...
    public function findingchart()
    {
        ...
        if (!empty($finding_types)) {
            foreach (
                $finding_types
                as $finding_type_key => $finding_type_value
            ) {
                $total_counts = $this->finding_model->getFindingCountbyPatients(
                    $patient_id,
                    $finding_type_value["name"],
                );
                $findings_array[] = [
    ...
    public function symptomchart()
    {
        ...
        if (!empty($symptoms)) {
            foreach ($symptoms as $symptom_key => $symptom_value) {
                $total_counts = $this->symptoms_model->getSymptomCountbyPatients(
                    $patient_id,
                    $symptom_value["symptoms_title"],
                );
                $symptoms_array[] = [
                    "total_counts" => $total_counts,
    ...
    public function yearchart()
    {
        ...
        $create_year = date(
            "Y",
            strtotime($patient_created) - 60 * 60 * 24 * 365,
        );
        $current_year = date("Y");
        $opd_visits = $this->patient_model->getpatientOPDYearCounter(
            $patient_id,
            $create_year,
        );
        $ipd_visits = $this->patient_model->getpatientIPDYearCounter(
            $patient_id,
            $create_year,
        );
        $pharmacy_visits = $this->pharmacy_model->getpatientPharmacyYearCounter(
            $patient_id,
            $create_year,
        );
        $pathology_visits = $this->pathology_model->getpatientPathologyYearCounter(
            $patient_id,
            $create_year,
        );
        $radiology_visits = $this->radio_model->getpatientRadiologyYearCounter(
            $patient_id,
            $create_year,
        );
        $bloodissue_visits = $this->bloodissue_model->getpatientBloodYearCounter(
            $patient_id,
            $create_year,
        );
        $ambulance_visits = $this->ambulance_model->getpatientAmbulanceYearCounter(
            $patient_id,
            $create_year,
        );
        ...
        if ($this->module_lib->hasPatientActive("opd")) {
            ...
            if (!empty($opd_visits)) {
                foreach ($year_range as $year_key => $year_value) {
                    ...
                    if (
                        !is_null(
                            searchForKeyData($year_value, $opd_visits, "year"),
                        )
                    ) {
                        $result_key = searchForKeyData(
                            $year_value,
                            $opd_visits,
                            "year",
                        );
                        $total_visits =
                            $opd_visits[$result_key]["total_visits"];
                    }
        ...
        if ($this->module_lib->hasPatientActive("ipd")) {
            ...
            if (!empty($ipd_visits)) {
                foreach ($year_range as $year_key => $year_value) {
                    ...
                    if (
                        !is_null(
                            searchForKeyData($year_value, $ipd_visits, "year"),
                        )
                    ) {
                        $result_key = searchForKeyData(
                            $year_value,
                        ...
                        $total_visits =
                            $ipd_visits[$result_key]["total_visits"];
                    }
            ...
            if (!empty($pharmacy_visits)) {
                foreach ($year_range as $year_key => $year_value) {
                    ...
                    if (
                        ...
                        $total_visits =
                            $pharmacy_visits[$result_key]["total_visits"];
                    }
            ...
            if (!empty($pathology_visits)) {
                foreach ($year_range as $year_key => $year_value) {
                    ...
                    if (
                        ...
                        $total_visits =
                            $pathology_visits[$result_key]["total_visits"];
                    }
            ...
            if (!empty($radiology_visits)) {
                foreach ($year_range as $year_key => $year_value) {
                    ...
                    if (
                        ...
                        $total_visits =
                            $radiology_visits[$result_key]["total_visits"];
                    }
            ...
            if (!empty($bloodissue_visits)) {
                foreach ($year_range as $year_key => $year_value) {
                    ...
                    if (
                        ...
                        $total_visits =
                            $bloodissue_visits[$result_key]["total_visits"];
                    }
            ...
            if (!empty($ambulance_visits)) {
                foreach ($year_range as $year_key => $year_value) {
                    ...
                    if (
                        ...
                        $total_visits =
                            $ambulance_visits[$result_key]["total_visits"];
                    }
    ...
    public function index()
    {
        ...
        $total_pharmacy = $this->pharmacy_model->totalPatientPharmacy(
            $patient_id,
        );
        $total_pathology = $this->pathology_model->totalPatientPathology(
            $patient_id,
        );
        $total_radiology = $this->radio_model->totalPatientRadiology(
            $patient_id,
        );
        $total_blood_issue = $this->bloodissue_model->totalPatientBloodIssue(
            $patient_id,
        );
        $total_ambulance = $this->ambulance_model->totalPatientAmbulance(
            $patient_id,
        );
        ...
        // ── Pulse layout — additional widget data ───────────────────
        $this->load->model("bill_model");
        $data["patient_detail"] = $patient_detail;
        $data["upcoming_appointments"] = $this->module_lib->hasPatientActive("my_appointments")
            ? $this->appointment_model->getPatientUpcomingAppointments($patient_id, 3)
            : array();
        $data["recent_prescriptions"] = $this->prescription_model->getPatientLastOpdPrescriptions($patient_id, 5);
        $data["pending_bills"] = $this->bill_model->getPatientModuleBalance($patient_id);
        $last_path = $this->module_lib->hasPatientActive("pathology")
            ? $this->pathology_model->getPatientLastOpdPathology($patient_id, 5)
            : array();
        foreach ($last_path as &$_p) { $_p["lab_type"] = "pathology"; }
        unset($_p);
        $last_radio = $this->module_lib->hasPatientActive("radiology")
            ? $this->radio_model->getPatientLastOpdRadiology($patient_id, 5)
            : array();
        foreach ($last_radio as &$_r) { $_r["lab_type"] = "radiology"; }
        unset($_r);
        $merged_labs = array_merge($last_path, $last_radio);
        usort($merged_labs, function ($a, $b) {
            return strcmp((string) $b["date"], (string) $a["date"]);
        });
        $data["pending_labs"] = array_slice($merged_labs, 0, 5);
    ...
    public function profile()
    {
        ...
        if (!empty($id)) {
            ...
            $timeline_list = $this->timeline_model->getPatientTimeline(
                $id,
                $timeline_status = "yes",
            );
            $prescription_details = $this->prescription_model->getopdvisitPrescription(
                $id,
            );
        }
        ...
        foreach ($opd_details as $key => $opdvalue) {
            $data[
                "opdconferences"
            ] = $this->conference_model->getconfrencebyopd(
                $opdvalue["staff_id"],
                $id,
            );
        }
        ...
        $data["fields"] = $this->customfield_model->get_custom_fields(
            "opd",
            "",
            "",
            "",
        ...
        $data[
            "investigations"
        ] = $this->patient_model->allinvestigationbypatientid($id);
        $data["recent_record_count"] = 5;
        $data["patientdetails"] = $this->patient_model->getpatientoverview($id);
        $data["timeformat"] = $this->time_format;
        $data["patientcurrentvital"] = $this->vital_model->getcurrentvitals(
            $id,
        );
        $data[
            "patient_vital_date"
        ] = $patient_vital_date = $this->vital_model->getpatientvitaldate($id);
        ...
        foreach ($patient_vital_date as $patient_vital_date_result) {
            if ($patient_vital_date_result["messure_date"]) {
                $messure_date = date(
                    "Y-m-d",
                    strtotime($patient_vital_date_result["messure_date"]),
                );
            } else {
                $messure_date = "";
            }
            ...
            foreach ($vital_list as $vital_list_result) {
                ...
                $patient_vital[$messure_date][
                    $vital_id
                ] = $this->vital_model->getpatientsvital(
                    $id,
                    $vital_id,
                    $messure_date,
                );
    ...
    {
        $report = $this->pathology_model->getPatientPathologyReportDetails(
            $report_id,
        );
        $this->load->helper("download");
    ...
    {
        $report = $this->radio_model->getPatientRadiologyReportDetails(
            $report_id,
        );
        $this->load->helper("download");
    ...
    public function printPatientPathologyReportDetail()
    {
        ...
        $data["bill_prefix"] = $this->customlib->getPatientSessionPrefixByType(
            "pathology_billing",
        );
        $data["result"] = $result;

        $page = $this->load->view(
            "patient/pathology/_printPatientReportDetail",
            $data,
            true,
        );
    ...
    public function printPatientRadiologyReportDetail()
    {
        ...
        $page = $this->load->view(
            "patient/radiology/_printPatientReportDetail",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function getDetails()
    {
        ...
        $result["age"] = $this->customlib->getPatientAge(
            $result["age"],
            $result["month"],
            $result["day"],
        );
        ...
        $appointment_date = $this->customlib->YYYYMMDDHisTodateFormat(
            $result["appointment_date"],
            $this->time_format,
        );
        $result["appointment_date"] = $appointment_date;
        $result["patient_name"] = composePatientName(
            $result["patient_name"],
            $result["patient_id"],
        );
        $result["doctor_name"] = composeStaffNameByString(
            $result["name"],
            $result["surname"],
            $result["employee_id"],
        );
    ...
    public function patient_discharge()
    {
        ...
        $type = $this->input->post("module_type", true);
        $data["result"] = $patient;
        $data["discharge_card"] = $this->patient_model->get_dischargeCard([
            "case_reference_id" => $case_id,
        ]);
        $page = $this->load->view(
            "patient/bill/_patient_discharge",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function print_dischargecard()
    {
        ...
        if (
            !empty($data["discharge_card"]) &&
            $data["discharge_card"]["discharge_status"] == "1"
        ) {
            $death_record = $this->birthordeath_model->getDeDetailsbycaseId(
                $case_id,
            );
            $id = $death_record["id"];
            $this->load->helper("customfield_helper");
            ...
            $deathrecord[
                "death_date"
            ] = $this->customlib->YYYYMMDDHisTodateFormat(
        ...
        $page = $this->load->view(
            "patient/bill/_printDischargeCard",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function getvisitDetails()
    {
        ...
        $result = $this->patient_model->getopdvisitDetailsbyvisitid($visitid);
        $result["opd_no"] =
            $this->customlib->getPatientSessionPrefixByType("opd_no") .
            $result["opdid"];
        $result["appointment_date"] = date(
            $this->customlib->getHospitalDateFormat(true, true),
            strtotime($result["appointment_date"]),
        );
        $result["doctor_name"] = composeStaffNameByString(
            $result["name"],
            $result["surname"],
    ...
    public function getopdrecheckupDetails()
    {
        $data = [];
        $visitid = $this->input->post("visit_id", true);
        $result = $this->patient_model->getopdvisitrecheckupDetailsbyvisitid(
            $visitid,
        );
        $data["fields"] = $this->customfield_model->get_custom_fields(
            "opdrecheckup",
        ...
        $data["result"] = $result;
        $page = $this->load->view(
            "patient/_getopdrecheckupDetails",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function appointment()
    {
        ...
        foreach ($resultlist as $key => $value) {
            ...
            foreach ($str_arr as $value) {
                $specialistname[] = $this->specialist_model->getspecialistType(
                    $value,
                );
            }
        ...
        $data[
            "appoint_priority_list"
        ] = $this->appoint_priority_model->appoint_priority_list();
        $this->load->view("layout/patient/header");
    ...
    public function bloodbank()
    {
        ...
        $data[
            "blood_issuefields"
        ] = $this->customfield_model->get_custom_fields(
            "blood_issue",
            "",
            "",
            "",
    ...
    public function liveconsult()
    {
        ...
        $patient_id = $this->patient_data["patient_id"];
        $data["id"] = $patient_id;
        $data["conferences"] = $this->conference_model->getconfrencebypatient(
            $patient_id,
        );
        $this->load->view("layout/patient/header");
    ...
    public function pharmacyvalidate()
    {
        $this->form_validation->set_rules(
            ...
            "required|valid_amount|trim|xss_clean|greater_than[0]",
        );
        $this->form_validation->set_rules(
            "payment_for",
            $this->lang->line("payment_for"),
    ...
    public function getlivestatus()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $array = ["status" => "fail", "error" => $data];
            echo json_encode($array);
        } else {
            $conference_id = $this->input->post("id", true);
            ...
            $data["page"] = $this->load->view(
                "patient/_livestatus",
                $data,
                true,
            );
            $array = ["status" => "1", "page" => $data["page"]];
    ...
    public function join($id)
    {
        ...
        $data["host"] =
            $live->create_for_surname == ""
                ? $live->create_for_name
                : $live->create_for_name . " " . $live->create_for_surname;
    ...
    public function add_history()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->conferencehistory_model->updatehistory(
                $data_insert,
                "patient",
            );
            $array = ["status" => 1, "error" => ""];
            echo json_encode($array);
        }
    }
    ...
    public function bookAppointment()
    {
        ...
        foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
            if ($custom_fields_value["validation"]) {
                ...
                $this->form_validation->set_rules(
                    "custom_fields[appointment][" . $custom_fields_id . "]",
                    $custom_fields_name,
                    "trim|required",
                );
            }
        ...
        $this->form_validation->set_rules(
            "doctor",
            $this->lang->line("doctor"),
            "required",
        );
        $this->form_validation->set_rules(
            "live_consult",
            $this->lang->line("live_consultation"),
            "required",
        );
        $this->form_validation->set_rules(
            "appointment_status",
            $this->lang->line("appointment_status"),
            "required",
        );
        if ($this->form_validation->run() == false) {
            $msg = [
            ...
            $appointment = [
                "patient_id" => $patient_id,
                "date" => $this->customlib->dateFormatToYYYYMMDDHis(
                    $date,
                    $this->time_format,
                ),
                ...
                "source" => "Online",
                "appointment_status" => $this->input->post(
                    "appointment_status",
                    true,
                ),
            ];
            ...
            if (!empty($custom_field_post)) {
                foreach ($custom_field_post as $key => $value) {
                    $check_field_type = $this->input->post(
                        "custom_fields[appointment][" . $key . "]",
                        true,
                    );
                    $field_value = is_array($check_field_type)
                        ? implode(",", $check_field_type)
                        : $check_field_type;
                    $array_custom = [
                        "belong_table_id" => 0,
                        "custom_field_id" => $key,
            ...
            if (!empty($custom_value_array)) {
                $this->customfield_model->insertRecord(
                    $custom_value_array,
                    $insert_id,
                );
            }
    ...
    public function bill()
    {
        ...
        $patient_id = $this->patient_data["patient_id"];
        $data["id"] = $patient_id;
        $data["resultlist"] = $this->pharmacy_model->getBillBasicPatient(
            $patient_id,
        );
        $data[
            "medicineCategory"
        ] = $this->medicine_category_model->getMedicineCategoryPat();
    ...
    public function getBillDetailsAmbulance($id)
    {
        ...
        $data["superadmin_restriction"] =
            $this->patient_data["superadmin_restriction"];
        $data["fields"] = $this->customfield_model->get_custom_fields(
            "ambulance_call",
            "",
            "",
        ...
        if (isset($_POST["print"])) {
            ...
            $this->load->view("patient/_billdetailAmbulance", $data);
        }
    }
    ...
    public function patientipddetails()
    {
        ...
        $patient_id = $this->patient_data["patient_id"];
        $data["resultlist"] = $this->patient_model->getipdpatientdetails(
            $patient_id,
        );
        $i = 0;
    ...
    public function ipdprofile($ipdid = "", $pres_id = "")
    {
        ...
        $id = $this->patient_data["patient_id"];
        if ($ipdid == "") {
            $ipdresult = $this->patient_model->search_ipd_patients(
                $searchterm = "",
                $active = "yes",
                $discharged = "no",
                $id,
            );
            $ipdid = !empty($ipdresult) ? $ipdresult["ipdid"] : "";
        }
        ...
        $data["ipdconferences"] = $this->conference_model->getconfrencebyipd(
            $doctors,
            $id,
            $ipdid,
        );
        ...
        if (!empty($id)) {
            ...
            $result = $this->patient_model->getIpdDetails(
                $ipdid,
                $status["is_active"],
            );
            if (!empty($result)) {
                ...
                $prescription_details = $this->prescription_model->getIpdPrescription(
                    $ipdid,
                );
                $consultant_register = $this->patient_model->getPatientConsultant(
                    $id,
                    $ipdid,
                );
                $charges = $this->charge_model->getCharges($ipdid);
                $paymentDetails = $this->transaction_model->IPDPatientPayments(
                    $ipdid,
                );
                $data[
                    "medicationreport_overview"
                ] = $this->patient_model->getmedicationdetailsbydate_overview(
                    $ipdid,
                );
                $paid_amount = $this->payment_model->getPaidTotal($id, $ipdid);
                $data["paid_amount"] = $paid_amount["paid_amount"];
                $nurse_note = $this->patient_model->getdatanursenote(
                    $id,
                    $ipdid,
                );
                $data["nurse_note"] = $nurse_note;
                foreach ($nurse_note as $key => $nurse_note_value) {
                    $notecomment = $this->patient_model->getnurenotecomment(
                        $ipdid,
                        $nurse_note_value["id"],
                    );
                ...
                $data["is_discharge"] = $this->customlib->checkDischargePatient(
                    $data["result"]["ipd_discharge"],
                );
                $data["prescription_detail"] = $prescription_details;
                $data["opd_details"] = $opd_details;
                $data["timeline_list"] = $timeline_list;
                ...
                $medicationreport = $this->patient_model->getmedicationdetailsbydate(
                    $ipdid,
                );
                $data["max_dose"] = $max_dose->max_dose;
                $data["medication"] = $medicationreport;
                $operation_theatre = $this->operationtheatre_model->getipdoperationDetails(
                    $ipdid,
                    "patient",
                );
                ...
                $data["bed_history"] = $this->bed_model->getBedHistory(
                    $case_reference_id,
                );
                $doctors_ipd = $this->patient_model->getDoctorsipd($ipdid);
                $data[
                    "discharge_card"
                ] = $this->patient_model->get_dischargeCard([
                    "case_reference_id" => $case_reference_id,
                ]);
                $data[
                    "fields_consultant"
                ] = $this->customfield_model->get_custom_fields(
                    "ipdconsultinstruction",
                    "",
                    "",
                    "",
                    1,
                ...
                $data[
                    "investigations"
                ] = $this->patient_model->getallinvestigation(
                    $result["case_reference_id"],
                );
                $data[
                    "graph"
                ] = $this->transaction_model->ipd_bill_paymentbycase_id(
                    $case_reference_id,
                );
                ...
                if ($data["result"]["ipdcredit_limit"] > 0) {
                    ...
                    if ($my_balance >= $data["credit_limit"]) {
                        ...
                        $credit_limit_percentage =
                            ($my_balance / $data["credit_limit"]) * 100;
                        $data["donut_graph_percentage"] = number_format(
                            100 - $credit_limit_percentage,
                            2,
                        );
                        $data["balance_credit_limit"] =
                            $data["credit_limit"] - $my_balance;
                ...
                $data[
                    "getipdoverviewtreatment"
                ] = $this->patient_model->getipdoverviewtreatment($id);
            } else {
        ...
        $data[
            "obstetric_history"
        ] = $this->antenatal_model->getobstetrichistory($id);
    ...
    public function getBillDetails()
    {
        ...
        $ipd_opd = $this->pharmacy_model->getIpdPrescriptionBasic(
            $ipd_prescription_basic_id,
        );
        if ($ipd_prescription_basic_id != "" && !empty($ipd_opd)) {
            if ($ipd_opd->ipd_id != "") {
                $data["prescription"] =
        ...
        $data["superadmin_restriction"] =
            $this->patient_data["superadmin_restriction"];
        $action_details .=
            "<a href='javascript:void(0)' class='btn btn-sm btn-light print_bill' data-bs-toggle='tooltip' title='" .
        ...
        $page = $this->load->view(
            "patient/pharmacy/_getBillDetailsPharma",
            $data,
            true,
        );
        echo json_encode([
    ...
    public function PrintBillDetailsPathology()
    {
        ...
        $page = $this->load->view(
            "patient/pathology/_getBillDetails",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function PrintBillDetailsRadiology()
    {
        ...
        $data["result"] = $result;
        $page = $this->load->view(
            "patient/radiology/_getBillDetails",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function add_patient_timeline()
    {
        ...
        $this->form_validation->set_rules(
            ...
            "callback_handle_doc_upload[timeline_doc]",
        );
        ...
        if ($this->form_validation->run() == false) {
            ...
            $array = ["status" => "fail", "error" => $msg, "message" => ""];
        } else {
            ...
            $timeline = [
                "title" => $this->input->post("timeline_title"),
                "timeline_date" => $this->customlib->dateFormatToYYYYMMDD(
                    $timeline_date,
                ),
    ...
    public function edit_patient_timeline()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (
                ...
                $upload_data = ["id" => $timelineid, "document" => $img_name];
                $this->timeline_model->add_patient_timeline($upload_data);
            }
            ...
            if ($files = filesize($_FILES[$var]["tmp_name"])) {
                if (!in_array($file_type, $allowed_mime_type)) {
                    $this->form_validation->set_message(
                        "handle_doc_upload",
                        $this->lang->line(
                            "file_type_extension_error_uploading_document",
                        ),
                    );
                    return false;
                }
                ...
                if (
                    !in_array($ext, $allowed_extension) ||
                    !in_array($file_type, $allowed_mime_type)
                ) {
                    $this->form_validation->set_message(
                        "handle_doc_upload",
                        $this->lang->line(
                            "extension_error_while_uploading_document",
                        ),
                    );
                    return false;
                }
                ...
                if ($file_size > 2097152) {
                    $this->form_validation->set_message(
                        "handle_doc_upload",
                        $this->lang->line("file_size_shoud_be_less_than") .
                            "2MB",
                    );
                    return false;
                }
                ...
                $this->form_validation->set_message(
                    "handle_doc_upload",
                    $this->lang->line("error_while_uploading_document"),
                );
                return false;
            }
    ...
    public function printTransaction()
    {
        ...
        if ($module_type == "opd") {
            $transaction = $this->transaction_model->opdPaymentByTransactionId(
                $id,
            );
        } else {
            $transaction = $this->transaction_model->ipdPaymentByTransactionId(
                $id,
            );
        }
        ...
        $data["print_details"] = $print_details;
        $page = $this->load->view(
            "patient/transaction/_printTransaction",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function getDetailsOt()
    {
        ...
        if (
            $result["patient_type"] == "Inpatient" ||
            $result["patient_type"] == "Outpatient"
        ) {
            $opd_ipd_no = $this->operationtheatre_model->getopdipdDetails(
                $id,
                $result["patient_type"],
            );
            $result["opd_ipd_no"] = $opd_ipd_no;
        }
        $result["admission_date"] = date(
            $this->customlib->getHospitalDateFormat(true, true),
            strtotime($result["admission_date"]),
        );
        $result["date"] = date(
            $this->customlib->getHospitalDateFormat(true, true),
    ...
    public function otsearch()
    {
        ...
        $data[
            "charge_category"
        ] = $this->operationtheatre_model->getChargeCategory();
        $data["resultlist"] = $this->operationtheatre_model->searchFullTextPat(
            $patient_id,
        );
        $data["organisation"] = $this->organisation_model->get();
        $this->load->view("layout/patient/header");
    ...
    public function getBillDetailsPatho($id, $parameter_id)
    {
        ...
        $parameterdetails = $this->pathology_category_model->getparameterDetailsforpatient(
            $id,
        );
    ...
    public function getBillDetailsRadio($id, $parameter_id)
    {
        ...
        $data["parametername"] = $parametername;
        $parameterdetails = $this->radio_model->getparameterDetailsforpatient(
            $id,
        );
        $data["parameterdetails"] = $parameterdetails;
    ...
    public function getPatientPathologyDetails()
    {
        ...
        $data["superadmin_restriction"] =
            $this->patient_data["superadmin_restriction"];
        $page = $this->load->view(
            "patient/pathology/_getPatientPathologyDetails",
            $data,
            true,
        );
        $actions =
            "<a href='javascript:void(0)' class='btn btn-sm btn-light print_parameter' data-bs-toggle='tooltip' title='" .
            $this->lang->line("print_test_report") .
            "' data-record-id='" .
            $id .
            "'><i class='fa fa-reorder'></i></a>";
        $actions .=
            "<a href='javascript:void(0)' class='btn btn-sm btn-light print_bill' data-bs-toggle='tooltip' title='" .
            $this->lang->line("print_bill") .
            "' data-record-id='" .
            $id .
            "'><i class='fa fa-print'></i></a>";
        echo json_encode([
    ...
    public function getPatientRadiologyDetails()
    {
        ...
        $page = $this->load->view(
            "patient/radiology/_getPatientRadiologyDetails",
            $data,
            true,
        );
        $actions = "";
    ...
    public function getConsultantBatch()
    {
        $patient_id = $this->patient_data["patient_id"];
        $data["id"] = $patient_id;
        $result = $this->operationtheatre_model->getConsultantBatchOt(
            $patient_id,
        );
        $data["result"] = $result;
    ...
    public function visitdetails($opd_details_id)
    {
        if (!empty($opd_details_id)) {
            ...
            $visit_details = $this->patient_model->getVisitDetails(
                $opd_details_id,
                "patient",
            );
            ...
            $paymentDetails = $this->transaction_model->OPDPatientPayments(
                $opd_details_id,
            );
            $data["payment_details"] = $paymentDetails;
            $data[
                "medicineCategory"
            ] = $this->medicine_category_model->getMedicineCategory();
            ...
            $paid_amount = $this->payment_model->getOPDPaidTotal(
                $opd_details_id,
            );
            $max_dose = $this->patient_model->getMaxByopdid($opd_details_id);
            $data["max_dose"] = $max_dose->max_dose;
            $medicationreport = $this->patient_model->getmedicationdetailsbydateopd(
                $opd_details_id,
            );
            $data["medication"] = $medicationreport;
            $data["billpaid_amount"] = 0;
            $operation_theatre = $this->operationtheatre_model->getopdoperationDetails(
                $opd_details_id,
                "patient",
            );
            $data["operation_theatre"] = $operation_theatre;
            ...
            $data["charge_type"] = $this->charge_type;
            $data["dosage"] = [];
            $getVisitDetailsid = $this->patient_model->getVisitDetailsid(
                $opd_details_id,
            );
            ...
            if (!empty($getVisitDetailsid)) {
                $data[
                    "visitconferences"
                ] = $this->conference_model->getconfrencebyvisitid(
                    $getVisitDetailsid,
                );
            }

            $data["fields_ot"] = $this->customfield_model->get_custom_fields(
                "operationtheatre",
                "",
                "",
                "",
            ...
            $data[
                "patientdetails"
            ] = $this->patient_model->getpatientoverviewbycaseid(
                $result["case_reference_id"],
            );
            $data[
                "medicationreport_overview"
            ] = $this->patient_model->getmedicationdetailsbydate_opdoverview(
                $opd_details_id,
            );

            $data[
                "graph"
            ] = $this->transaction_model->opd_bill_paymentbycase_id(
                $result["case_reference_id"],
            );
            $data["recent_record_count"] = 5;
            $data["superadmin_restriction"] =
            ...
            $data[
                "patient_vital_date"
            ] = $patient_vital_date = $this->vital_model->getpatientvitaldate(
                $result["pid"],
            );
            $data[
                "vital_list"
            ] = $vital_list = $this->vital_model->getvitallist();
    ...
    public function download_patient_timeline($timeline_id)
    {
        ...
        $this->media_storage->filedownload(
            $timeline_list["document"],
            "./uploads/patient_timeline/",
        );
    }
    ...
    public function getIpdDetails()
    {
        ...
        $result["patient_age"] = $this->customlib->getPatientAge(
            $result["age"],
            $result["month"],
            $result["day"],
        );
        ...
        $result["bed_group"] =
            $result["bedgroup_name"] . "-" . $result["floor_name"];
        $data["fields"] = $this->customfield_model->get_custom_fields(
            "patient",
            "",
            "",
    ...
    public function deleteappointment($id)
    {
        if ($id != "") {
            ...
            $json_array = [
                "status" => "1",
                "error" => "",
                "msg" => $this->lang->line("delete_message"),
            ];
        } else {
            $json_array = ["status" => "0", "error" => "", "message" => ""];
        }
    ...
    public function user_language($lang_id)
    {
        $language_name = $this->db
            ->select("languages.language,languages.is_rtl,languages.short_code")
            ->from("languages")
            ->where("id", $lang_id)
            ->get()
            ->row_array();
        ...
        $language_array = [
            ...
            "short_code" => $language_name["short_code"],
        ];
    ...
    public function otdetails()
    {
        ...
        $data[
            "operation_theater_reference_no"
        ] = $this->customlib->getPatientSessionPrefixByType(
            "operation_theater_reference_no",
        );
        $page = $this->load->view(
            "admin/operationtheatre/_otdetails",
            $data,
            true,
        );

        $actions =
            "<a href='javascript:void(0)' data-loading-text='<i class=\"fa fa-circle-o-notch fa-spin\"></i>' class='btn btn-sm btn-light print_ot_bill'  data-bs-toggle='tooltip' data-record-id=\"" .
            $ot_id .
    ...
    public function print_otdetails()
    {
        ...
        $data["print_details"] = $print_details;
        $id = $this->input->post("id");
        $data["otdetails"] = $this->operationtheatre_model->otdetailsforprint(
            $id,
            "patient",
        );
        $data["fields"] = $this->customfield_model->get_custom_fields(
        ...
        $action =
            '<a href="javascript:void(0);" class=" print_dischargecard" data-bs-toggle="tooltip" title="" data-module_type="' .
            $type .
            '"  data-recordId="' .
            $id .
            '" data-original-title=""><i class="fa fa-print"></i> </a>';
        $page = $this->load->view("patient/_printotdetails", $data, true);
        echo json_encode(["status" => 1, "page" => $page, "action" => $action]);
    }
    ...
    public function getinvestigationparameter()
    {
        ...
        if ($lab == "pathology") {
            $actions = "";
            $id = $_REQUEST["id"];
            $result = $this->pathology_model->getPatientPathologyReportDetails(
                $id,
            );
            $data["result"] = $result;
            $page = $this->load->view(
                "patient/pathology/_labinvestigations",
                $data,
                true,
            );
            ...
            if ($result->pathology_report != "") {
                $actions =
                    "<a href='" .
                    base_url() .
                    "patient/dashboard/downloadPathologyReport/" .
                    $id .
                    "' class='btn btn-sm btn-light' data-bs-toggle='tooltip' title='" .
                    $this->lang->line("download") .
                    "'><i class='fa fa-download'></i></a>";
            }

            $actions .=
                "<a href='javascript:void(0)' data-loading-text='<i class=\"fa fa-circle-o-notch fa-spin\"></i>' class='btn btn-sm btn-light print_bill' data-bs-toggle='tooltip' data-record-id=\"" .
                $id .
                "\" data-type-id='" .
                $lab .
                "' title='" .
            ...
            $page = $this->load->view(
                "patient/radiology/_labinvestigations",
                $data,
                true,
            );
            ...
            if ($result->radiology_report != "") {
                $actions =
                    "<a href='" .
                    base_url() .
                    "patient/dashboard/downloadRadiologyReport/" .
                    $id .
                    "' class='btn btn-sm btn-light' data-bs-toggle='tooltip' title='" .
                    $this->lang->line("download") .
                    "'><i class='fa fa-download'></i></a>";
    ...
    public function printpathoparameter()
    {
        ...
        if ($lab == "pathology") {
            ...
            $data["result"] = $result;
            $page = $this->load->view(
                "patient/pathology/_printlabinvestigations",
                $data,
                true,
            );
            echo json_encode(["status" => 1, "page" => $page]);
            ...
            $data["result"] = $result;
            $page = $this->load->view(
                "patient/radiology/_printlabinvestigations",
                $data,
                true,
            );
            echo json_encode([
    ...
    public function getopdtreatmenthistory()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $check = $this->db
                    ->where("visit_details_id", $visit_details_id)
                ...
                if ($check->num_rows() > 0) {
                    ...
                    if ($this->session->has_userdata("hospitaladmin")) {
                        $doctor_restriction =
                            $this->session->userdata["hospitaladmin"][
                                "doctor_restriction"
                            ];
                ...
                $action .=
                    "<a href='javascript:void(0)' data-loading-text='" .
                    $this->lang->line("please_wait") .
                    "' data-opd-id=" .
                    $opd_id .
                    " data-record_id=" .
                    $visit_details_id .
                    " class='btn btn-outline-secondary btn-sm get_opd_detail' data-bs-toggle='tooltip' title='" .
                    $this->lang->line("show") .
                    "'><i class='fa fa-reorder'></i></a>";
                $action .= "</div>";
    ...
    public function getipdtreatmenthistory($id)
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $id = $value->id;
                $ipdid = $value->ipdid;
                $discharge_details = $this->patient_model->getIpdBillDetails(
                    $id,
                    $ipdid,
                );
                ...
                $action .=
                    "<a href=" .
                    base_url() .
                    "admin/patient/ipdprofile/" .
                    $value->ipdid .
                    " class='btn btn-default btn-xs'  data-bs-toggle='tooltip' title='" .
                    $this->lang->line("show") .
                    "'><i class='fa fa-reorder' aria-hidden='true'></i></a>";
                ...
                $row[] =
                    $this->customlib->getPatientSessionPrefixByType("ipd_no") .
                    $value->ipdid;
                $row[] = $value->symptoms;
                $row[] = composeStaffNameByString(
                    $value->name,
    ...
    {
        $id = (int) $this->input->post("appointment_id", true);
        ...
        if ($result["start_time"]) {
            if ($this->time_format) {
                $result["doctor_shift_name"] =
                    date("G:i:s", strtotime($result["start_time"])) .
                    " - " .
                    date("G:i:s", strtotime($result["end_time"]));
            } else {
                $result["doctor_shift_name"] =
                    date("g:i A", strtotime($result["start_time"])) .
                    " - " .
                    date("g:i A", strtotime($result["end_time"]));
            }
        } else {
            $result["doctor_shift_name"] = "";
        }

        $result["patients_name"] = composePatientName(
            $result["patients_name"],
            $result["patient_id"],
        );
        $result["edit_live_consult"] = $this->lang->line(
            $result["live_consult"],
        );
        $result["live_consult"] = $result["live_consult"];
        $result["date"] = $this->customlib->YYYYMMDDHisTodateFormat(
            $result["date"],
            $this->time_format,
        );
        $result["custom_fields_value"] = display_custom_fields(
            "appointment",
            $id,
        );
        $cutom_fields_data = get_custom_table_values($id, "appointment");
        $result["field_data"] = $cutom_fields_data;
        $result["patients_gender"] = $this->lang->line(
            strtolower($result["patients_gender"]),
        );
        ...
        if ($result["appointment_status"] == "approved") {
            ...
            if (
                $result["payment_mode"] != "" ||
                $result["payment_mode"] != null
            ) {
                $result["payment_mode"] = $this->lang->line(
                    strtolower($result["payment_mode"]),
                );
            } else {
                $result["payment_mode"] = "";
            }
        ...
        $result["patient_age"] = $this->customlib->get_patient_current_age(
            $result["patient_id"],
        );

        if ($result["cheque_date"]) {
            $result["cheque_date"] = $this->customlib->YYYYMMDDTodateFormat(
                $result["cheque_date"],
            );
        } else {
            $result["cheque_date"] = "";
        ...
        if ($result["attachment"] != "") {
            $result["doc"] =
                "<a href='" .
                site_url("patient/dashboard/downloadreceipt/") .
                (int) $transaction_id .
        ...
        $result["appointment_status"] = $this->lang->line(
            strtolower($result["appointment_status"]),
        );

        if ($result["source"]) {
            $result["source"] = $this->lang->line(
                strtolower($result["source"]),
            );
        } else {
            $result["source"] = "";
        }
    ...
    public function printAppointmentBill()
    {
        ...
        $result = $this->appointment_model->getDetailsAppointment(
            $id,
            $is_patient,
        );
        ...
        if ($result["appointment_status"]) {
            $result["appointmentstatus"] = $this->lang->line(
                strtolower($result["appointment_status"]),
            );
        } else {
    ...
    {
        $fields = $this->customfield_model->get_custom_fields(
            "component_issue",
            "",
            "",
            "",
        ...
        $dt_response = $this->bloodissue_model->getcomponentissuerecordById(
            $patient_id,
            "",
            "",
            "",
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action .=
                    "<a href='#' data-record-id='" .
                    $value->id .
                    "' class='btn btn-outline-secondary btn-sm viewcomponentDetail' data-bs-toggle='tooltip' title='" .
                    $this->lang->line("show") .
                    "'><i class='fa fa-reorder'></i></a>";
                $action .=
                    "<a href='#' data-record-id='" .
                    $value->id .
                    "' class='btn btn-outline-secondary view_payment btn-sm' data-bs-toggle='tooltip' data-module_type='blood_bank' title='" .
                    $this->lang->line("view_payments") .
                    "'><i class='fa fa-money'></i></a>";
                $action .=
                    '<a href="#" data-record-id="' .
                    $value->id .
                    '" class="printcomponentIssueBill btn btn-outline-secondary btn-sm" data-bs-toggle="tooltip" title="' .
                    $this->lang->line("print") .
                    '"><i class="fa fa-print"></i></a>';

                $action .=
                    '<a class="btn btn-primary btn-sm" data-bs-toggle="tooltip" title="' .
                    $this->lang->line("pay") .
                    '" onclick="payModal(' .
                    $value->id .
                    "," .
                ...
                $row[] = $prefix;
                $row[] = $value->case_reference_id;
                $row[] = $this->customlib->dateyyyymmddToDateTimeformat(
                    $value->date_of_issue,
                    false,
                );
    ...
    public function printBloodIssueBill()
    {
        ...
        $id = $this->input->post("id");
        $charge = [];
        $blood_issues_detail = $this->bloodissue_model->printBloodIssueBill(
            $id,
        );
        $transaction = $this->transaction_model->bloodbankPaymentByTransactionId(
            $id,
        );
        $data["print_details"] = $print_details;
        $data["blood_issues_detail"] = $blood_issues_detail;
        $data["transactions"] = $this->transaction_model->BloodBankPayments(
            $id,
        );
        $prefix = $this->customlib->getPatientSessionPrefixByType(
            "blood_bank_billing",
        );
        $data["prefix"] = $prefix;
        $data["fields"] = $this->customfield_model->get_custom_fields(
    ...
    public function printcomponentIssueBill()
    {
        ...
        $page = $this->load->view(
            "patient/_printcomponentIssueBill",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function getbloodissueDatatable($patient_id)
    {
        ...
        $dt_response = $this->bloodissue_model->getpatientbloodissueRecord(
            $patient_id,
        );
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action .=
                    "<a href='#' data-record-id='" .
                    $value->id .
                    "' class='btn btn-outline-secondary btn-sm viewDetail' data-bs-toggle='tooltip' title='" .
                    $this->lang->line("show") .
                    "'><i class='fa fa-reorder'></i></a>";
                $action .=
                    "<a href='#' data-record-id='" .
                ...
                $action .=
                    "<a href='#' data-record-id='" .
                    $value->id .
                    "' class='btn btn-outline-secondary btn-sm printIssueBill' data-bs-toggle='tooltip' title='" .
                    $this->lang->line("print") .
                    "'><i class='fa fa-print'></i></a>";
                $action .=
                    '<a class="btn btn-primary btn-sm" data-bs-toggle="tooltip" title="' .
    ...
    public function getpayment()
    {
        ...
        if ($module_type == "pharmacy") {
            $payment_details = $this->transaction_model->pharmacypaymentbybillid(
                $id,
                $patient_id,
            );
        } elseif ($module_type == "pathology") {
            $payment_details = $this->transaction_model->pathologypaymentbybillid(
                $id,
                $patient_id,
            );
        } elseif ($module_type == "radiology") {
            $payment_details = $this->transaction_model->radiologypaymentbybillid(
                $id,
                $patient_id,
            );
        } elseif ($module_type == "ambulance") {
            $payment_details = $this->transaction_model->ambulancepaymentbybillid(
                $id,
                $patient_id,
            );
        } elseif ($module_type == "blood_bank") {
            $payment_details = $this->transaction_model->bloodissuepaymentbybillid(
                $id,
                $patient_id,
            );
        }
    ...
    public function printbilltransaction()
    {
        ...
        if ($module_type == "pharmacy") {
            $transaction = $this->transaction_model->pharmacyPaymentByTransactionId(
                $id,
            );
        } elseif ($module_type == "pathology") {
            $transaction = $this->transaction_model->pathologyPaymentByTransactionId(
                $id,
            );
        } elseif ($module_type == "radiology") {
            $transaction = $this->transaction_model->radiologyPaymentByTransactionId(
                $id,
            );
        } elseif ($module_type == "ambulance") {
            $transaction = $this->transaction_model->ambulanceCallPaymentByTransactionId(
                $id,
            );
        } elseif ($module_type == "blood_bank") {
            $transaction = $this->transaction_model->bloodbankPaymentByTransactionId(
    ...
    public function addappointment()
    {
        ...
        // SaaS: load the quota library here (not autoloaded on the patient panel)
        // so the validateCanAddNewResource callback below has $this->saasvalidation
        // available during form_validation->run(). CI's loader is idempotent.
        ...
        $this->data["page"] = [
            "title" => $this->lang->line("appointment"),
            "meta_title" => "",
            "meta_keyword" => "",
            "meta_description" => "",
        ];
        ...
        $params =
            $this->input->post("doctor", true) .
            "," .
            $this->input->post("shift", true) .
            "," .
            $this->customlib->dateFormatToYYYYMMDD(
                $this->input->post("date", true),
            ) .
            "," .
            $this->input->post("global_shift", true);
        $custom_fields = $this->customfield_model->getByBelongPatientPanel(
            "appointment",
        );
        foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
            if ($custom_fields_value["validation"]) {
        ...
        $this->form_validation->set_rules(
            "slot",
            $this->lang->line("available_slots"),
            "trim|required|callback_check_slot[" . $params . "]",
        );
        $this->form_validation->set_rules(
            "message",
            $this->lang->line("message"),
        ...
        // SaaS count-limit pre-check (no_of_appointment). Virtual field with no input;
        // blocks the booking only when the tenant is over its appointment quota.
        $this->form_validation->set_rules(
            "validate_resource",
            $this->lang->line("appointment"),
            "callback_validateCanAddNewResource[no_of_appointment,1]",
        );
        ...
        if ($this->form_validation->run() == false) {
            $msg = [
                ...
                // SaaS quota-limit message; the view's $.each(data.error) toast surfaces it.
            ...
            $json_array = ["status" => "0", "error" => $error_msg];
            echo json_encode($json_array);
        } else {
            ...
            $date = $this->customlib->dateFormatToYYYYMMDD(
                $this->input->post("date", true),
            );
            $shift = $this->input->post("shift", true);
            $doctor = $this->input->post("doctor", true);
            $global_shift = $this->input->post("global_shift", true);
            ...
            $slot = $slots[$this->input->post("slot", true)];
            $time = date("H:i:s", strtotime($slot));
            $day = date("l", strtotime($date));
            $date_create = date("Y-m-d H:i:s");
            $getDoctorShiftTimeId = $this->onlineappointment_model->getDoctorShiftTimeId(
                $doctor,
                $global_shift,
                $day,
            );
            ...
            $appointment = [
                ...
                "rejected_time" => date(
                    "Y-m-d H:i:s",
                    strtotime("+10 minutes", strtotime($date_create)),
                ),
            ];

            $appointments = $this->onlineappointment_model->checkAppointmentsSlotBooked(
                $this->input->post("doctor"),
                $getDoctorShiftTimeId->id,
                $date . " " . $time,
                $current_time,
                0,
            );
            ...
            if (!empty($appointments)) {
                echo json_encode([
                    "status" => 2,
                    "msg" =>
                        "Appointment not available, Please try after some time.",
                ]);
            } else {
                ...
                $insert_id = $this->onlineappointment_model->addAppointment(
                    $appointment,
                );

                // SaaS: bump no_of_appointment usage by 1 only on a real insert (this
                // branch runs only when the slot was free). Own try/catch so a quota-API
                // hiccup never aborts the patient's already-booked appointment.
                if ($insert_id) {
                    try {
                        $this->saasvalidation->updateResouceQuota("no_of_appointment", 1);
                    } catch (Exception $e) {
                        log_message("error", "SaaS no_of_appointment quota increment failed (patient addappointment): " . $e->getMessage());
                    }
                }
                ...
                $date_appoint = $this->customlib->dateFormatToYYYYMMDDHis(
                    $date . " " . $time,
                    $this->customlib->getHospitalTimeFormat(),
                );
                $doctor_details = $this->notificationsetting_model->getstaffDetails(
                    $this->input->post("doctor", true),
                );
                $event_data = [
                    "appointment_date" => $this->customlib->YYYYMMDDHisTodateFormat(
                    ...
                    "doctor_id" => $this->input->post("doctor", true),
                    "doctor_name" => composeStaffNameByString(
                        $doctor_details["name"],
                        $doctor_details["surname"],
                        $doctor_details["employee_id"],
                    ),
                    "message" => $this->input->post("message", true),
                ];

                $this->system_notification->send_system_notification(
                ...
                $json_array = ["status" => 1, "msg" => "Appointment Booked"];
                echo json_encode($json_array);
            }
        }
    }
    ...
    /**
     * SaaS count-limit pre-check (form_validation callback). Returns false to block
     * the booking when one more appointment would push the tenant over its quota.
     * Param string is "resource_name,quantity"; SaasValidation sets the error message.
     */
    ...
    public function check_slot($slot, $params)
    {
        if ($slot == "") {
            $this->form_validation->set_message(
                "check_slot",
                $this->lang->line("available_slots_field_is_required"),
            );
            return false;
        }
        [$doctor_id, $shift, $date, $global_shift] = explode(",", $params);
        $appointments = $this->onlineappointment_model->getAppointments(
            $doctor_id,
            $shift,
            $date,
        );
        ...
        if ($slot != "" && $doctor_id != "" && $shift != "" && $date != "") {
            if (count($time) > $slot) {
                ...
                if (in_array($shift_time, $array)) {
                    $this->form_validation->set_message(
                        "check_slot",
                        $this->lang->line("this_slot_is_already_booked"),
                    );
                    return false;
                } else {
                    return true;
    ...
    {
        $id = (int) $this->input->post("blood_issue_id", true);
        ...
        $action = "";
        $action .=
            "<a href='javascript:void(0)' class='btn btn-sm btn-light printIssueBill' data-bs-toggle='tooltip' title='" .
            $this->lang->line("print") .
            "' data-record-id='" .
            $id .
            "'><i class='fa fa-print'></i></a>";
    ...
    public function getComponentIssueDetail()
    {
        ...
        $data["prefix"] = $this->customlib->getpatientSessionPrefixByType(
            "blood_bank_billing",
        );
        $data["fields"] = $this->customfield_model->get_custom_fields(
            "component_issue",
        ...
        $action = "";
        $action .=
            "<a href='javascript:void(0)' class='btn btn-sm btn-light printcomponentIssueBill' data-bs-toggle='tooltip' title='" .
            $this->lang->line("print") .
            "' data-record-id='" .
            $id .
            "'><i class='fa fa-print'></i></a>";
        $page = $this->load->view(
            "patient/_getcomponentIssueDetail",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page, "action" => $action]);
    ...
    public function printtestparameterdetail()
    {
        ...
        $data["head_result"] = $this->pathology_model->getPathologyBillByID(
            $id,
        );
        $data["result"] = $result;
        $data["bill_prefix"] = $this->customlib->getPatientSessionPrefixByType(
        ...
        $page = $this->load->view(
            "patient/pathology/_printtestparameterdetail",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function printradiotestparameterdetail()
    {
        ...
        $data["result"] = $result;
        $page = $this->load->view(
            "patient/radiology/_printtestparameterdetail",
            $data,
            true,
        );
        echo json_encode(["status" => 1, "page" => $page]);
    ...
    public function contentlist()
    {
        ...
        $data["superadmin_restriction"] =
            $this->patient_data["superadmin_restriction"];
        $data["contentlist"] = $this->sharecontent_model->getPatientShareList(
            $patient_id,
        );
        $data["timeformat"] = $this->time_format;
        $this->load->view("layout/patient/header");
    ...
    public function viewcontent($id)
    {
        ...
        $data[
            "content"
        ] = $this->sharecontent_model->getShareContentWithDocuments($id);
        $data["timeformat"] = $this->time_format;
        $this->load->view("layout/patient/header");
    ...
    public function deleteAccount()
    {
        ...
        $patient_id = (int) $patient_session["patient_id"];

        if (empty($patient_id)) {
            echo json_encode([
                "status" => "fail",
                "message" => $this->lang->line('invalid_session'),
            ]);
            return;
        }

        $result = $this->patient_model->deletePatient($patient_id);

        if ($result !== false) {
            $this->auth->logout();
            echo json_encode([
                "status" => "success",
                "message" => $this->lang->line('your_account_has_been_deleted'),
            ]);
        } else {
            echo json_encode([
                "status" => "fail",
                "message" => $this->lang->line('warning_message'),
            ]);
        }
    }
}

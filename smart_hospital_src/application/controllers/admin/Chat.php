...
class Chat extends Admin_Controller
{
    ...
    {
        $chat_connection_id = $this->input->post('chat_connection_id', TRUE);
        $chat_to_user       = $this->input->post('chat_to_user', TRUE);
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($user_type == "Patient") {
                if(isset($new_user->image)  && $new_user->image!=null){
                    $new_user->image= $this->media_storage->getImageURL($new_user->image);
                }else{
                    $new_user->image=$this->media_storage->getImageURL('uploads/patient_images/no_image.png');
                }
            } elseif ($user_type == "Staff") {
                if(isset($new_user->image) && $new_user->image!=null){

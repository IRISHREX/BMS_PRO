...
class Dispatch extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('to_title', $this->lang->line('to_title'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                try {
                    ...
                    log_message('error', 'SaaS storage quota update failed (dispatch index add): ' . $e->getMessage());
                }
            }
    ...
    public function getdispatchdatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($value->image !== "") {
                    $action .= "<a href='" . base_url() . 'admin/dispatch/download/' . $value->id . "' class='btn btn-sm btn-outline-secondary' data-bs-toggle='tooltip' data-bs-placement='top' title='" . $this->lang->line('download') . "'><i class='fa fa-download'></i></a>";
                }
    ...
    /**
     * SaaS: release a dispatch record's image from the storage quota. Returns the
     * image filename so the caller can delete the physical file (model->delete
     * does not unlink it).
     */
    private function releaseDispatchImageQuota($id)
    {
        $existing  = $this->dispatch_model->dis_rec_data($id, 'dispatch');
        ...
        if (!empty($old_image)) {
            ...
            if ($kb > 0) {
                try {
                    ...
                    log_message('error', 'SaaS storage quota release failed (dispatch): ' . $e->getMessage());
                }
            }
        }
        return $old_image;
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                try {
                    ...
                    log_message('error', 'SaaS storage quota update failed (dispatch add): ' . $e->getMessage());
                }
            }
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                if (!IsNullOrEmptyString($file_name)) {
                    ...
                    try {
                        ...
                        log_message('error', 'SaaS storage quota update failed (dispatch edit): ' . $e->getMessage());
                    }
                }
            }
    ...
    public function delete($id)
    {
        ...
        $img = $this->releaseDispatchImageQuota($id);
        if (!empty($img)) {
    ...
    public function imagedelete($id)
    {
        ...
        $this->releaseDispatchImageQuota($id);

        ...
        if (isset($total_rows) && !empty($total_rows)) {          
          ...
          //validation to check each input keeps the value or not
            foreach ($total_rows as $row_key => $row_value) {  
                $this->form_validation->set_rules('incomehead_'.$row_value, $this->lang->line('income_head'), 'trim|required|xss_clean');
            ...
            foreach ($total_rows as $row_key => $row_values) {                 
                $data  = array(
                    'income_category' => $this->input->post('incomehead_'.$row_values, TRUE),

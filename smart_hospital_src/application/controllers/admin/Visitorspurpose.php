...
class Visitorspurpose extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('visitors_purpose', $this->lang->line('visitors_purpose'), 'required|xss_clean');
        if ($this->form_validation->run() == false) {
            ...
            $visitors_purpose = array(
                'visitors_purpose' => $this->input->post('visitors_purpose', TRUE),
        ...
        if (isset($total_rows) && !empty($total_rows)) {          
          ...
          //validation to check each input keeps the value or not
            foreach ($total_rows as $row_key => $row_value) {  
                $this->form_validation->set_rules('visitors_purpose_'.$row_value, $this->lang->line('purpose'), 'trim|required|xss_clean');        
            ...
            foreach ($total_rows as $row_key => $row_values) {                 
                $data  = array(
                    'visitors_purpose' => $this->input->post('visitors_purpose_'.$row_values, TRUE),
                    'description' => $this->input->post('description_'.$row_values, TRUE));
    ...
    {
        $this->form_validation->set_rules('visitors_purpose', $this->lang->line('purpose'), 'required|xss_clean');

...
<div class="sh-form-card mb-3">
    ...
    <div class="p-2">
        ...
        <?php if (!empty($prescription_test->tests)) { ?>
            ...
            <table class="table table-sm table-striped table-bordered table-hover tblProducts sh-radio-edit-table mb-0" id="tableID">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr id="row<?php echo $counter; ?>">
                        <td>
                            ...
                            <select class="form-control form-control-sm test_name select2 w-100"  name="test_name_<?php echo $counter; ?>">
                                ...
                                <?php foreach ($radiology_tests as $radiology_test_key => $radiology_test_value) { ?>
                                    <option value="<?php echo $radiology_test_value["id"]; ?>" <?php echo ($radiology_test_value["id"] == $test_value->radiology_id) ? 'selected' : ''; ?>>
                                        <?php echo $radiology_test_value["test_name"] . " (" . $radiology_test_value["short_name"] . ")"; ?>
                                    </option>
                                <?php } ?>
                            </select>
                        ...
                        <td>
                            <input type="text" name="reportday_<?php echo $counter; ?>" id="reportday_<?php echo $counter; ?>" value="<?php echo $test_value->radio_report_days; ?>" class="form-control form-control-sm text-end days" readonly>
                            <span class="text-danger"><?php echo form_error('reportday[]'); ?></span>
                        </td>
                        <td>
                            <input type="text" name="reportdate_<?php echo $counter; ?>" id="reportdate_<?php echo $counter; ?>" value="<?php echo ($date == "") ? "" : $this->customlib->strtotimeToDateFormat($date + ($test_value->radio_report_days * 86400)); ?>" class="form-control form-control-sm text-end report_date date no-past-date" data-min-date="today">
                            <span class="text-danger"><?php echo form_error('reportdate[]'); ?></span>
                        </td>
                        <td>
                            <div class="input-group input-group-sm">
                                <input type="text" class="form-control form-control-sm text-end taxpercent" name="taxpercent_<?php echo $counter; ?>" id="taxpercent_<?php echo $counter; ?>" value="<?php echo $test_value->radiology_tax; ?>" readonly autocomplete="off">
                            ...
                            <input type="hidden" name="taxamount_<?php echo $counter; ?>" id="taxamount_<?php echo $counter; ?>" value="<?php
                                if ($is_tpa == 1) {
                                    if ($test_value->organisation_id == 0 || strtotime($insurance_validity) < strtotime(date('Y-m-d'))) {
                                        echo $test_value->radio_standard_charge * $test_value->radiology_tax / 100;
                                    } else {
                                        echo ($test_value->radio_org_charge_org_charge == null || $test_value->radio_org_charge_org_charge == 0) ? $test_value->radio_standard_charge * $test_value->radiology_tax / 100 : $test_value->radio_org_charge_org_charge * $test_value->radiology_tax / 100;
                                    }
                                } else {
                                    echo $test_value->radio_standard_charge * $test_value->radiology_tax / 100;
                                }
                        ...
                        <td class="text-center align-middle">
                            <button type="button" class="btn btn-sm btn-outline-danger delete_row" data-row-id="<?php echo $counter; ?>"><i class="fa fa-remove"></i></button>
                        </td>
                    </tr>
                    <?php $counter++; } ?>
                </tbody>
                ...
                <tbody id="addrows">
                    <tr id="row1">
                        <td>
                            ...
                            <select class="form-control form-control-sm test_name select2 w-100"  name="test_name_1">
                                <option value=""><?php echo $this->lang->line('select'); ?></option>
                ...
                <div class="row g-2">
                    <div class="col-sm-6">
                        <div class="mb-3">
                            ...
                            <select name="consultant_doctor" id="consultant_doctor" onchange="get_Docname(this.value)" class="form-control consultant_doctor select2 w-100" <?php if ($disable_option == true) { echo "disabled"; } ?>>
                                <option value=""><?php echo $this->lang->line('select'); ?></option>
    ...
    <div class="sh-flex-col-260" id="showamtdiv">
        <div class="sh-form-card h-100 overflow-hidden">
            ...
            <div class="d-flex align-items-center justify-content-between px-3 py-2 border-bottom">
                ...
                <input type="text" value="0" name="total" id="total" class="form-control form-control-sm text-end total sh-mw-130"  readonly />
            </div>
            <div class="d-flex align-items-center justify-content-between px-3 py-2 border-bottom">
                ...
                <div class="d-flex gap-2">
                    ...
                    <input type="text" value="0" onkeyup="get_percentage(this.value)" name="discount" id="discount" class="form-control form-control-sm text-end discount sh-w-130" />
                </div>
            </div>
            <div class="d-flex align-items-center justify-content-between px-3 py-2 border-bottom">
                <span class="text-secondary small fw-semibold"><?php echo $this->lang->line('tax') . " (" . $currency_symbol . ")"; ?></span>
                <input type="text" value="0" name="tax" id="tax" class="form-control form-control-sm text-end tax sh-mw-130"  readonly />
            </div>
            <div class="d-flex align-items-center justify-content-between px-3 py-2 border-bottom">
                <span class="text-secondary small fw-semibold"><?php echo $this->lang->line('net_amount') . " (" . $currency_symbol . ")"; ?></span>
                <input type="text" value="0" name="net_amount" id="net_amount" class="form-control form-control-sm text-end text-secondary fw-semibold net_amount sh-mw-130"  readonly />
            </div>
            <div class="px-2 py-3">
                <div class="row g-2">
                    <div class="col-6">
